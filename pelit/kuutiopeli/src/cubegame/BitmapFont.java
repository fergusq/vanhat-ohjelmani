...
public class BitmapFont {
	...
	private final String allowedChar = Messages.getString("BitmapFont.allowedChars"); //$NON-NLS-1$
	
	/**
	 * Create a new font based on specific texture cut up into a specific
	 * collection of characters
	 * 
	 * @param fontTexture The texture containing the characters
	 ...
	 */
	public BitmapFont(cubegame.Texture fontTexture, int characterWidth, int characterHeight) {
		...
		characterWidthInTexture = fontTexture.getWidth() / (fontTexture.getImageWidth() / characterWidth);
		characterHeightInTexture = fontTexture.getHeight() / (fontTexture.getImageHeight() / characterHeight);
	
		// work out the number of characters that fit across the sheet
		charactersAcross = fontTexture.getImageWidth() / characterWidth;
	 ...
	 * @param characterHeight The height of the characters on the sheet (in pixels)
	 */
	public BitmapFont(cubegame.Texture fontTexture, int characterWidth, int characterHeight, int scale) {
		this.texture = fontTexture;
		this.characterWidth = characterWidth;
		this.characterHeight = characterHeight;
		...
		// chosen an arbitary value here to move the letters a bit
		// closer together when rendering them
		characterStep = (characterWidth - 5) / scale;
		
		this.characterHeight = characterHeight / scale;
		this.characterWidth = characterWidth / scale;
	}
	 ...
	 */
	public synchronized void drawString(int font, String text, int x, int y) {
