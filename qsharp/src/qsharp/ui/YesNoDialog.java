...
* described. 
*/
public class YesNoDialog implements WidgetsConstants, ItemListener {
    ...
    */
	public YesNoDialog(String title, String text, String yes, String no) {
		_title = title;
		this.text = text;
		this.yes = yes;
		this.no = no;
	...
	*/
	public void show() {
		...
		for (int i = 0; i < sa.length; i++) if (sa[i].length()> width) width = sa[i].length();
		width += 4;
		...
		_peer = new Dialog((Toolkit.getScreenWidth()-width)/2,(Toolkit.getScreenHeight()-height)/2,width,
								 height,
								 true,_title);
		GridLayoutManager manager1 = new GridLayoutManager(1,2);
		_peer.getRootPanel().setLayoutManager(manager1);
		manager1.addWidget(_label,0,0,1,1,ALIGNMENT_CENTER,ALIGNMENT_CENTER);
		manager1.addWidget(_menuList,0,1,1,1,ALIGNMENT_CENTER,ALIGNMENT_CENTER);
